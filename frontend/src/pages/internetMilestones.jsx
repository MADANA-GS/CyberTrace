...
const InternetEvolutionTimeline = () => {
  ...
  // Initialize the buttonRefs array
  useEffect(() => {
    buttonRefs.current = Array(filteredMilestones.length)
      .fill()
      .map((_, i) => buttonRefs.current[i] || React.createRef());
  }, [filteredMilestones.length]);

  // Initialize filtered milestones
  useEffect(() => {
    setFilteredMilestones(internetMilestones);
  }, []);
  ...
  useEffect(() => {
    ...
    const filtered = internetMilestones.filter(
      (milestone) =>
        milestone.title.toLowerCase().includes(query) ||
        milestone.description.toLowerCase().includes(query) ||
        milestone.year.toLowerCase().includes(query) ||
        milestone.impact.toLowerCase().includes(query)
    );
  ...
  useEffect(() => {
    let interval;
    if (autoPlay && filteredMilestones.length > 0) {
      interval = setInterval(() => {
        setActiveIndex((current) => (current + 1) % filteredMilestones.length);
      }, 5000); // 5 seconds per milestone
    }
    return () => clearInterval(interval);
  }, [autoPlay, filteredMilestones.length]);

  // Update active decade when milestone changes
  useEffect(() => {
    if (filteredMilestones.length > 0) {
      const year = parseInt(filteredMilestones[activeIndex].year);
      const decade = Math.floor(year / 10) * 10;
      setActiveDecade(`${decade}s`);
    }
  }, [activeIndex, filteredMilestones]);

  // Scroll timeline to center active button when activeIndex changes
  useEffect(() => {
    if (
      timelineContainerRef.current &&
      buttonRefs.current[activeIndex] &&
      buttonRefs.current[activeIndex].current &&
      viewMode === "timeline"
    ) {
      const container = timelineContainerRef.current;
      const activeButton = buttonRefs.current[activeIndex].current;
  ...
  const handlePrevious = () => {
    if (filteredMilestones.length > 0) {
      setActiveIndex(
        (current) =>
          (current - 1 + filteredMilestones.length) % filteredMilestones.length
      );
    }
  };
  ...
  const jumpToMilestone = (index) => {
    setActiveIndex(index);

    // Scroll to detailed view
    if (detailedViewRef.current) {
      setTimeout(() => {
  ...
  // Redirect handler for "Read full article"
  const handleReadFullArticle = () => {
    // This would normally use react-router or window.location
    // For demonstration, we'll just show how it would be structured
    if (filteredMilestones.length > 0) {
  ...
  const groupedByDecade = filteredMilestones.reduce((decades, milestone) => {
    const decade = Math.floor(parseInt(milestone.year) / 10) * 10;
    const decadeKey = `${decade}s`;
    if (!decades[decadeKey]) {
      decades[decadeKey] = [];
    }
    decades[decadeKey].push(milestone);
    return decades;
  }, {});
  ...
  return (
    <div className="flex flex-col items-center bg-gradient-to-br from-gray-900 to-black text-white p-2 sm:p-4">
      {/* Main Content Area */}
      <div className="w-full max-w-6xl mx-auto">
        {/* Title Section */}
        <div className="text-center mb-8 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-4 sm:mb-6 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-600">
            The Evolution of the Internet
          </h1>
          <p className="text-base sm:text-lg md:text-xl max-w-4xl mx-auto text-gray-300 px-2">
            From a military research project to a global digital ecosystem that
            has transformed every aspect of human society.
          </p>
        </div>
        ...
        <div className="w-full max-w-lg mx-auto mb-6 sm:mb-8">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2 sm:py-3 bg-gray-900 bg-opacity-70 text-white border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500 transition-colors"
            />
            {searchQuery && (
              <button
                onClick={clearSearch}
          ...
          {searchQuery && (
            <p className="text-sm text-gray-400 mt-2">
              Found {filteredMilestones.length}{" "}
              {filteredMilestones.length === 1 ? "result" : "results"}
            </p>
          )}
        </div>
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="flex space-x-2 sm:space-x-4">
            <button
              ...
              className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-md border border-blue-500 text-blue-400 hover:bg-blue-900 hover:bg-opacity-30 transition-colors text-sm sm:text-base"
            >
              View as {viewMode === "timeline" ? "Grid" : "Timeline"}
            </button>
            <button
              onClick={() => setAutoPlay(!autoPlay)}
              className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-md border text-sm sm:text-base ${
                autoPlay
        ...
        {filteredMilestones.length === 0 ? (
          <div className="text-center p-8 bg-gray-900 bg-opacity-60 rounded-lg">
          ...
          <>
            {/* Timeline View - Redesigned for better scrolling */}
            {viewMode === "timeline" && (
              <div className="mb-8 sm:mb-12">
                {/* Decade-based navigation - scrollable on mobile */}
                <div className="flex items-center mb-6 overflow-x-auto py-2 no-scrollbar w-full justify-center">
                  {sortedDecades.map((decade) => (
                    <div key={decade} className="px-1 sm:px-2 flex-shrink-0">
                      <button
                        className={`px-2 py-1 sm:px-3 sm:py-2 rounded-lg text-sm sm:text-base ${
                          decade === activeDecade
                        ...
                        onClick={() =>
                          jumpToMilestone(
                            filteredMilestones.findIndex(
                              (m) => m.year === groupedByDecade[decade][0].year
                            )
                          )
                        }
                      >
                ...
                {/* Year navigation - Scrollable timeline with fixed-width buttons */}
                <div className="relative">
                  <div
                    ref={timelineContainerRef}
                    className="flex items-center py-4 overflow-x-auto no-scrollbar"
                    style={{
                      minHeight: "60px",
                      position: "relative",
                      scrollbarWidth: "none",
                    ...
                    <div className="flex space-x-2 px-12">
                      {filteredMilestones.map((milestone, index) => (
                        <button
                          ...
                          className={`px-2 py-1.5 sm:px-3 cursor-pointer sm:py-2 whitespace-nowrap rounded-md transition-all duration-300 text-xs sm:text-sm md:text-base ${
                            index === activeIndex
                          ...
                          style={{
                            minWidth: "150px", // Fixed minimum width for all buttons
                  ...
                  <div className="flex justify-between absolute top-1/2 left-0 right-0 transform -translate-y-1/2 pointer-events-none px-1">
                    <button
                      onClick={handlePrevious}
                      className="w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-black bg-opacity-50 border border-blue-800 flex items-center justify-center hover:bg-blue-900 hover:bg-opacity-50 transition-colors pointer-events-auto z-10"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-8 sm:mb-12">
                {filteredMilestones.map((milestone, index) => (
                  ...
                  >
                    ...
                    <h2 className="text-lg cursor-pointer sm:text-xl font-bold mb-1 sm:mb-2">
                      {milestone.title}
                    </h2>
                    <p className="text-gray-300 text-xs sm:text-sm mb-2 sm:mb-3 line-clamp-2">
                      {milestone.description}
                    </p>
                    <p className="text-blue-400 text-xs sm:text-sm italic line-clamp-1">
                      {milestone.impact}
                    </p>
                  </div>
                ))}
            ...
            {/* Detailed Milestone View - Improved for mobile */}
            {filteredMilestones.length > 0 && (
              <div
                ref={detailedViewRef}
                className="relative w-full max-w-4xl mx-auto my-6 sm:my-8 p-4 sm:p-8 rounded-xl bg-black bg-opacity-60 backdrop-blur-md border border-blue-800 shadow-2xl"
                key={`milestone-${activeIndex}`}
                style={{
                ...
                id="detailed-view"
              >
                <span className="absolute -top-3 -left-3 sm:-top-5 sm:-left-5 text-3xl sm:text-5xl">
                  {filteredMilestones[activeIndex].icon}
                </span>
                <div className="flex flex-col sm:flex-row justify-between items-start mb-4 sm:mb-6">
                  <div>
                    <div    onClick={handleReadFullArticle} className="flex items-baseline flex-wrap">
                      ...
                      <span className="px-2 py-0.5 sm:px-3 sm:py-1 bg-blue-900 rounded-full text-blue-300 text-xs sm:text-sm mt-1 sm:mt-2 md:mt-0">
                        {filteredMilestones[activeIndex].year}
                      </span>
                    </div>
                    <div className="w-24 sm:w-32 h-1 bg-gradient-to-r from-blue-600 to-purple-600 mt-2 sm:mt-3 mb-3 sm:mb-4"></div>
                  </div>
                ...
                <p className="text-base sm:text-lg md:text-xl text-gray-200 mb-4 sm:mb-6 leading-relaxed">
                  {filteredMilestones[activeIndex].description}
                </p>

                <div className="bg-blue-900 bg-opacity-20 border-l-4 border-blue-500 p-3 sm:p-4 rounded">
                  <h3 className="text-blue-400 font-medium mb-1 sm:mb-2 text-sm sm:text-base">
                    Historical Impact
                  </h3>
                  <p className="text-gray-300 text-sm sm:text-base">
                    {filteredMilestones[activeIndex].impact}
                  </p>
                </div>

                <div className="mt-6 sm:mt-8 flex justify-between flex-wrap">
                  <span className="text-gray-500 text-xs sm:text-sm mb-2 md:mb-0">
                    Milestone {activeIndex + 1} of {filteredMilestones.length}
                    {searchQuery && ` (filtered by search)`}
                  </span>
                  <button
                    onClick={handleReadFullArticle}
                    className="flex items-center text-blue-400 hover:text-blue-300 transition-colors text-xs sm:text-sm"
                  >
                    <span>Read full article</span>
                    <ExternalLink size={12} className="ml-1 sm:hidden" />
                    <ExternalLink size={14} className="ml-1 hidden sm:block" />
                  </button>
                </div>
              </div>
            )}
        ...
        {/* Context Section - Improved for mobile */}
        <div className="w-full max-w-4xl mx-auto my-8 sm:my-12 p-4 sm:p-6 rounded-lg bg-black bg-opacity-50 backdrop-blur-sm border border-blue-900 shadow-lg">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-medium mb-2 sm:mb-4">
            From <span className="text-blue-400">Military Networks</span> to{" "}
            <span className="text-purple-400">The Metaverse</span>
          </h2>
          <p className="text-gray-300 text-sm sm:text-base leading-relaxed">
            The internet has evolved from a small research project connecting a
            handful of universities to a global infrastructure supporting
            billions of users and devices. Each era has built upon the
            innovations of the previous, creating an ever-more-complex ecosystem
            that continues to transform how we work, connect, and live.
          </p>
        </div>
      </div>
...
export default InternetEvolutionTimeline;
